...
const Business = (props) => {
  const showBlocks = (data) =>
    data.map((item, index) => {
      return (
        <Col key={`block${index}`} className='block' md={4} xs={8} {...item.colProps}>
          <Link className='content5-block-content' to={item.link ? item.link : ''}>
  ...
  const childrenToRender = showBlocks(
